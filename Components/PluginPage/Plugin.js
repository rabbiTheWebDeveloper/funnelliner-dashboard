...
const Plugin = ({ setFetch }) => {
    ...
    const addonsInstallVerify = (id, addon) => {
        ...
        const myAddonButton = Array.isArray(myAddonsList) ? myAddonsList.find(a => a?.addons_details?.id === id) : null
        if (myAddonButton?.addons_details?.id === id) {
            ...
            return (
                <Button disabled={isLoading} onClick={() => handleInstallAddons(id)}>
                    {setIsLoading && clickedAddonID == id ? <Bars
                        height="30"
                        width="60"
                        radius="9"
                        color="#fff"
    ...
    const addonsInstallVerifyWithPay = (id, addon) => {
        ...
        if (myAddonButton?.addons_details?.id === id) {
            ...
            return (
                <Button onClick={e => { setModalOpen(true); setPaymentValue(addon?.amount); setAddonsId(addon?.id) }}>Pay Now</Button>
    ...
    return (
        ...
        <>
            ...
            <section className='DashboardSetting Plugin'>
                ...
                {/* header */}
                <HeaderDescription headerIcon={'flaticon-prototype'} title={'Addons'} subTitle={'Add plugins for extra advantages'} search={false}></HeaderDescription>
                                    ...
                                    <TabContext value={value}>
                                        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
                                            ...
                                            <div className="AddonsTabsHeader d_flex d_justify CommonTab">
                                                ...
                                                <TabList onChange={handleChange} aria-label="lab API tabs example">
                                                    <Tab icon={<TbPlugConnectedX />} iconPosition="start" label="All Addons" value="1" />
                                                    <Tab icon={<MdDashboardCustomize />} iconPosition="start" label="My Addons" value="2" />
                                                </TabList>
                                        ...
                                        <TabPanel value="2">
                                            ...
                                            <Grid container spacing={3}>
                                                ...
                                                {
                                                    Array.isArray(myAddonsList) ? myAddonsList.map((Addons, index) => {

                                                        return (

                                                            <Grid item xs={12} sm={6} md={4} key={index}>

                                                                <div className={`AddonsTabItem boxShadow commonCart ${index == 0 && "cart-1" } ${index == 1 && "cart-2" } ${index == 2 && "cart-3" } ${index == 3 && "cart-4" } ${index == 4 && "cart-5" }`}>

                                                                    <div className={Addons?.addons_details?.payment_type === "free" ? "PainUnpaid Free" : "PainUnpaid"}>
                                                                        <h5>{Addons?.addons_details?.payment_type}</h5>
                                                                    </div>
                                                                    <div className="img">
                                                                        <img src={Addons.addons_image_details?.name} alt={Addons?.addons_details?.name} />
                                                                    </div>
                                                                    <div className="text">
                                                                        ...
                                                                        <div className="DualButton">
                                                                            {
                                                                                Addons?.status === 0 ?
                                                                                    <>
                                                                                        <Button onClick={() => addonsActive(Addons?.id)} >Active</Button>
                                                                                        <Button className='Deactivated' onClick={() => addonsUninstall(Addons.id)}>Uninstall</Button>
                                                                                    </>
                                                                                    :
                                                                                    <Button className='Deactivated' onClick={() => addonsActive(Addons?.id)} >Deactivate</Button>
                                        ...
                                        <TabPanel value="1">
                                            ...
                                            <Grid container spacing={3}>
                                                ...
                                                {
                                                    addonsList.length > 0 && addonsList.map((addon, index) => {
                                                        return (
                                                            ...
                                                            <Grid item xs={12} sm={6} md={4} key={index}>
                                                                ...
                                                                <div className={`AddonsTabItem boxShadow commonCart ${index == 0 && "cart-1" } ${index == 1 && "cart-2" } ${index == 2 && "cart-3" } ${index == 3 && "cart-4" } ${index == 4 && "cart-5" }`}>
                                                                    ...
                                                                    <div className={addon?.payment_type === "free" ? "PainUnpaid Free" : "PainUnpaid"}>
                                                                        <h5>{addon?.payment_type}</h5>
                                                                    </div>
                                                                    <div className="img">
                                                                        <img src={addon?.addons_image?.name} alt="" />
                                                                    </div>
                                                                    <div className="text">
                                                                        <h4>{addon?.name}</h4>
                                                                        <h5>{addon?.amount}.00 Tk</h5>
                                                                        <div className="DualButton">
                                                                            {addon?.payment_type === "free" ? addonsInstallVerify(addon?.id, addon)
                                                                                : addonsInstallVerifyWithPay(addon.id, addon)}
                                                                        </div>
                                                                    </div>
                                                                </div>
                                    ...
                                    <div className="SubscriptionModal">
                                        ...
                                        <div className="PaymentType">
                                            ...
                                            <label className="card">
                                                <input name="plan" className="radio" type="radio" checked={selectedGender === "bkash"} value="bkash" onChange={handleGenderChange} />
                                                <img src="/images/payment-img/bkash.png" alt="" />
                                            </label>

                                            <label className="card">
                                                <input name="plan" className="radio" type="radio" checked={selectedGender === "nagad"} value="nagad" onChange={handleGenderChange} />
                                                <img src="/images/payment-img/nagod.png" alt="" />
                                            </label>

                                            <label className="card">
                                                <input name="plan" className="radio" type="radio" checked={selectedGender === "ssl"} value="ssl" onChange={handleGenderChange} />
                                        ...
                                        <div className="Review">
                                            ...
                                            <div className="SubscribeAmount">
                                                ...
                                                <h3><i className="flaticon-taka" />{paymentValue}</h3>
                                            </div>
                                        ...
                                        <div className="duelButton">
                                            <Button onClick={() => handlePaymentMethod(selectedGender)}>Pay Now</Button>
                                        </div>
